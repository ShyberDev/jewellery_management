...
def create_order_stock_out_on_submit(
    doc,
    method=None,
):
    """
    Create Stock OUT transactions when a Jewellery Order
    is submitted after reaching Delivered.

    Only items previously transferred into Retail Stock
    receive Stock OUT.

    Items delivered directly without Stock IN do not
    receive Stock OUT.
    """
    ...
    for row in doc.jewellery_items:
        ...
        if not row.stock_transferred:
            continue

        if row.stock_delivered:
            continue

        if not row.stock_transaction:
            frappe.throw(
                f"Stock transaction is missing for "
                f"order item {row.idx}."
            )
        ...
        existing_transaction = frappe.db.exists(
            "Jewellery Stock Transaction",
            {
                ...
                "transaction_type": "Order Transfer",
                "item": row.item_name,
                "retail_stock_item": row.retail_stock_item,
                "movement": "Out",
            },
        )
        ...
        if existing_transaction:
            ...
            row.stock_delivered = 1
            row.delivery_stock_transaction = (
                existing_transaction
            )
